...
static int wpa_driver_tista_set_band(void *priv, u32 band)
{
	int res = 0;
	u32 config_band = band;
	struct wpa_driver_ti_data *drv = (struct wpa_driver_ti_data *)priv;

	switch(config_band){
		case WIFI_FREQUENCY_BAND_AUTO:
			config_band = RADIO_BAND_DUAL;
			break;
		case WIFI_FREQUENCY_BAND_5GHZ:
			config_band = RADIO_BAND_5_0_GHZ;
			break;
		case WIFI_FREQUENCY_BAND_2GHZ:
			config_band = RADIO_BAND_2_4_GHZ;
			break;
		default:
			res = -1;
			wpa_printf(MSG_ERROR," ERROR - Unknown band %d", config_band);
			break;
	}

	drv->band = config_band;

	return res;
}

static int wpa_driver_tista_get_band(void *priv, u32* band)
{
	int res = 0;
	struct wpa_driver_ti_data *drv = (struct wpa_driver_ti_data *)priv;
	...
	switch(drv->band){
		...
		case RADIO_BAND_5_0_GHZ:
			*band = WIFI_FREQUENCY_BAND_5GHZ;
			break;
		case RADIO_BAND_2_4_GHZ:
			*band = WIFI_FREQUENCY_BAND_2GHZ;
			break;
		default:
			res = -1;
	}
...
static int get_num_of_channels(char *country)
{
	...
	if ( (os_strcasecmp(country, "EU") == 0) ||
	     (os_strcasecmp(country, "NL") == 0) ||
	     (os_strcasecmp(country, "CZ") == 0) ||
	     (os_strcasecmp(country, "AT") == 0) ||
	     (os_strcasecmp(country, "GB") == 0) ||
	     (os_strcasecmp(country, "DE") == 0) ||
	     (os_strcasecmp(country, "KR") == 0) ||
	     (os_strcasecmp(country, "CN") == 0) )
		channels = NUMBER_SCAN_CHANNELS_ETSI;
        else if (os_strcasecmp(country, "JP") == 0)
...
static int wpa_driver_tista_driver_cmd( void *priv, char *cmd, char *buf, size_t buf_len )
{
	...
	if( os_strcasecmp(cmd, "reload") == 0 ) {
		...
		int t;
		band = (u32)atoi(cmd + 7);
		wpa_printf(MSG_DEBUG,"setband command %u", band);
		if (band <= RADIO_BAND_NUM_OF_BANDS) {
			ret = wpa_driver_tista_set_band( priv, band );
		}
	}
	else if( os_strncasecmp(cmd, "getband",7) == 0 ) {
		u32 band = 0;
		wpa_printf(MSG_DEBUG,"getband command %d", band);
		wpa_driver_tista_get_band( priv, &band );
		ret = sprintf(buf, "Band = %u\n", band);
	}
...
void * wpa_driver_tista_init(void *ctx, const char *ifname)
{
	...
	/* Set default band to auto */
	drv->band = RADIO_BAND_DUAL;
