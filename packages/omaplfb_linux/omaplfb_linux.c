...
static OMAPLFB_BOOL isUsingVirtualPanel(OMAPLFB_DEVINFO *psDevInfo)
{
	OMAPLFB_BOOL ret = OMAPLFB_FALSE;
	unsigned i;
	struct omapfb_info *ofbi = FB2OFB(psDevInfo->psLINFBInfo);
	struct omapfb2_device *dev = ofbi->fbdev;
	int nbDisplays = dev->num_displays;

	for (i=0; i < nbDisplays; i++)
	{
		struct omap_dss_device *disp = dev->displays[i];
		if (disp->panel.is_virtual) ret = OMAPLFB_TRUE;
	}
	return ret;
}
...
static void OMAPLFBEarlySuspendHandler(struct early_suspend *h)
{
	...
	for (i=0; i < uiMaxFBDevIDPlusOne; i++)
	{
		...
		if (psDevInfo != NULL)
		{
			...
			if (isUsingVirtualPanel(psDevInfo) == OMAPLFB_FALSE) OMAPLFBBlankDisplay(psDevInfo);
		}
	}
}
...
static void OMAPLFBEarlyResumeHandler(struct early_suspend *h)
{
	...
	for (i=0; i < uiMaxFBDevIDPlusOne; i++)
	{
		...
		if (psDevInfo != NULL)
		{
			if (isUsingVirtualPanel(psDevInfo) == OMAPLFB_FALSE) OMAPLFBUnblankDisplay(psDevInfo);
