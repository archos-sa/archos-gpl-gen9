			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (bss) {
		bss_wapi = wpa_bss_get_vendor_ie(bss, WAPI_IE_VENDOR_TYPE);
		if (bss_wapi == NULL)
	...
	if (bss_wapi) {
		wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: bss info is available");
		if (wapi_parse_ie(bss_wapi, 2 + bss_wapi[1], &ie))
			return -1;

		if ((ssid->key_mgmt & ie.akm_suite) & WPA_KEY_MGMT_WAPI_PSK)
			wpa_s->key_mgmt = WPA_KEY_MGMT_WAPI_PSK;
		else if ((ssid->key_mgmt & ie.akm_suite) &
				WPA_KEY_MGMT_WAPI_CERT)
			wpa_s->key_mgmt = WPA_KEY_MGMT_WAPI_CERT;
		else {
			wpa_printf(MSG_ERROR, "WAPI: Failed to "
					"select key management.");
			return -1;
		}
	} else { /* no 'bss' is available (ap_scan=2 or hidden ssid).
			take information from configuration MCS00068132 */
		wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: bss info is not-available");
		wpa_s->key_mgmt = ssid->key_mgmt;
	}

	if (wapi_gen_ie(wpa_ie, wpa_ie_len, wpa_s->key_mgmt,
			WPA_CIPHER_SMS4, WPA_CIPHER_SMS4)) {
		wpa_msg(wpa_s, MSG_ERROR, "WAPI: error generating ie");
		return -1;
	}
	if (wapi_set_assoc_ie(wpa_s->wapi, wpa_ie, *wpa_ie_len)) {
		wpa_msg(wpa_s, MSG_ERROR, "WAPI: error saving ie");
		return -1;
	}
 ...
 */
void wpa_supplicant_rx_wai(void *ctx, const u8 *src_addr,
			     const u8 *buf, size_t len) {
	struct wpa_supplicant *wpa_s = ctx;
	struct wapi_sm *sm = wpa_s->wapi;

	wapi_rx_wai(sm, src_addr, buf, len);
}
#endif /* CONFIG_WAPI */
...
#ifdef CONFIG_WAPI
	if (wpa_s->wapi_l2 &&
			l2_packet_get_own_addr(wpa_s->wapi_l2,
			wpa_s->own_addr)) {
		wpa_msg(wpa_s, MSG_ERROR, "Failed to get own L2 address for WAPI");
		return -1;
	}
#endif /* CONFIG_WAPI */
