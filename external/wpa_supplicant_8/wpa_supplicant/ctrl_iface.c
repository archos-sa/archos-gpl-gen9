...
#ifdef CONFIG_WAPI
static int ctrl_iface_get_status(struct wpa_supplicant *wpa_s, char *buf,
				 size_t buflen, int verbose)
{
	...
	ret = os_snprintf(pos, end - pos,
			  ...
			  wpa_key_mgmt_txt(wpa_s->key_mgmt, proto));
	...
	*/
	...
	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		...
		if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ||
			...
			pos += ctrl_iface_get_status(wpa_s, pos,
								end - pos,
								verbose);
		} else
#endif /* CONFIG_WAPI */
				    ...
				    const u8 *ie, size_t ie_len)
{
	...
	if (wapi_parse_ie(ie, ie_len, &wie)) {
		wpa_printf(MSG_ERROR, "WAPI: error parsing ie");
		ret = os_snprintf(pos, end - pos, "?]");
		if (ret < 0 || ret >= end - pos)
			return pos;
		pos += ret;
...
#ifdef CONFIG_WAPI
	ie3 = wpa_bss_get_vendor_ie(bss, WAPI_IE_VENDOR_TYPE);
	if (!ie3)
