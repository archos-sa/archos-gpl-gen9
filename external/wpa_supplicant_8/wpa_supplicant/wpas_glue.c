...
#ifdef CONFIG_WAPI
/**
 * wapi_ether_send - Send Ethernet frame
 * @wpa_s: Pointer to wpa_supplicant data
 * @dest: Destination MAC address
 * @proto: Ethertype in host byte order
 * @buf: Frame payload starting from IEEE 802.1X header
 ...
 * Returns: >=0 on success, <0 on failure
 */
static int wapi_ether_send(struct wpa_supplicant *wpa_s, const u8 *dest,
			  const u8 *buf, size_t len)
{
	if (wpa_s->wapi_l2)
		return l2_packet_send(wpa_s->wapi_l2, dest,
				WAPI_ETHER_TYPE, buf, len);

	return wpa_drv_send_eapol(wpa_s, dest, WAPI_ETHER_TYPE, buf, len);
}
#endif /* CONFIG_WAPI */
...
#ifdef CONFIG_WAPI
static int _wapi_ether_send(void *wpa_s, const u8 *dest,
			   const u8 *buf, size_t len)
{
	return wapi_ether_send(wpa_s, dest, buf, len);
}
#endif /* CONFIG_WAPI */
...
#ifdef CONFIG_WAPI
static void wapi_set_supplicant_port(void *ctx, int authorized)
{
	struct wpa_supplicant *wpa_s = ctx;
	wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: Supplicant port status: %s",
...
int wpa_supplicant_init_wapi(struct wpa_supplicant *wpa_s)
{
	...
	ctx = os_zalloc(sizeof(*ctx));
	if (ctx == NULL) {
		wpa_msg(wpa_s, MSG_ERROR, "Failed to allocate WAPI context.");
		return -1;
	}
	...
	ctx->deauthenticate = _wpa_supplicant_deauthenticate;
	ctx->set_key = wpa_supplicant_set_key;
	ctx->ether_send = _wapi_ether_send;
	ctx->cancel_auth_timeout = _wpa_supplicant_cancel_auth_timeout;
	ctx->cancel_scan = _wpa_supplicant_cancel_scan;
	ctx->set_port = wapi_set_supplicant_port;

	wpa_s->wapi = wapi_sm_init(NULL, ctx);
	if (wpa_s->wapi == NULL) {
		wpa_msg(wpa_s, MSG_ERROR, "Failed to initialize WAPI state "
			   "machine");
		return -1;
	}
	return 0;
...
#ifdef CONFIG_WAPI
void wpa_supplicant_wapi_set_config(struct wpa_supplicant *wpa_s,
					struct wpa_ssid *ssid)
{
	struct wapi_ssid_config conf;
	if (ssid) {
		os_memset(&conf, 0, sizeof(conf));
		conf.id = ssid->id;
