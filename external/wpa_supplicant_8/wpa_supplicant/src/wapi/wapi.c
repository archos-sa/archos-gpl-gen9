...
static int wapi_answer_ukey_req(struct wapi_sm *sm, struct wapi_msg *msg)
{
	...
	if (sm->cur_ssid && (sm->cur_ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK)) {
		...
		if (sm->cur_ssid->wapi_auth & WAPI_AUTH_PSK_ASCII) {
			...
			wpa_hexdump(MSG_DEBUG, "WAPI HEX PSK",
					sm->cur_ssid->wapi_psk_hex,
					sm->cur_ssid->wapi_psk_hex_len);
...
size_t wapi_file_size(FILE *fp) {
	...
	if (fstat(fileno(fp), &file_stat) != 0) {
		wpa_printf(MSG_ERROR, "WAPI: error retreiving file stat");
		return -1;
	}
...
int wapi_retrieve_cert(struct wapi_sm *sm) {
	int ret_val = -1;
	size_t cert_len = 0, root_len = 0;
	FILE *fcert = NULL, *froot = NULL;

	u8 cert[WAPI_CERT_LEN];
	u8 root[WAPI_CERT_LEN];
	...
	if ((!sm->cur_ssid->wapi_user_cert) || (!sm->cur_ssid->wapi_root_cert)) {
		wpa_printf(MSG_ERROR,
	...
	fcert = fopen(sm->cur_ssid->wapi_user_cert, "rb");
	froot = fopen(sm->cur_ssid->wapi_root_cert, "rb");

	if (!fcert || !froot) {
		wpa_printf(MSG_ERROR, "WAPI: error opening certificate files");
		goto end;
	}

	cert_len = wapi_file_size(fcert);
	root_len = wapi_file_size(froot);

	if (cert_len == -1 || root_len == -1) {
		wpa_printf(MSG_ERROR, "WAPI: error retreiving file stats");
		goto end;
	}

	if ((fread(cert, 1, cert_len, fcert) != cert_len) ||
		(fread(root, 1, root_len, froot) != root_len)) {
		wpa_printf(MSG_ERROR, "WAPI: error reading certificate files");
		goto end;
	}

	if (wapi_ec_extract_pkey_from_PEM((char *)cert, cert_len,
			sm->asue_priv_key)) {
		wpa_printf(MSG_ERROR, "WAPI: error extracting private key");
		goto end;
	}

	if (wapi_ec_X509_PEM_to_DER((char *)cert, cert_len,
...
void wapi_sm_set_config(struct wapi_sm *sm, struct wapi_ssid_config *config)
{
	...
	if (config) {
		...
		sm->cur_ssid->wapi_user_cert = config->wapi_user_cert;
		sm->cur_ssid->wapi_root_cert = config->wapi_root_cert;
		os_memcpy(sm->cur_ssid->wapi_psk_hex, config->wapi_psk_hex,
				config->wapi_psk_hex_len);
		sm->cur_ssid->wapi_psk_hex_len = config->wapi_psk_hex_len;
		sm->cur_ssid->wapi_auth = config->wapi_auth;
	} else {
		if (sm->cur_ssid)
			os_free(sm->cur_ssid);
		sm->cur_ssid = NULL;
