...
#define  INVALID_PLAY_POS   0xFFFFFFFF
...
static gboolean notify_position(struct avrcp_player *player)
{
	uint32_t position = player->cb->get_position(player->user_data);

	if (position == player->last_sent_pos)
		return FALSE;
	avrcp_player_event(player, AVRCP_EVENT_PLAYBACK_POS_CHANGED, &position);
...
int avrcp_player_event(struct avrcp_player *player, uint8_t id, void *data)
{
	...
	/* We need to send AVRCP_EVENT_PLAYBACK_POS_CHANGED on every change we currntly support */
	if (id != AVRCP_EVENT_PLAYBACK_POS_CHANGED  && (player->registered_events & (1 << AVRCP_EVENT_PLAYBACK_POS_CHANGED)))
         {
               player->last_sent_pos = INVALID_PLAY_POS;
		       notify_position(player);
         }
	...
	case AVRCP_EVENT_PLAYBACK_POS_CHANGED:
		size = 5;
		position =  *(uint32_t *)data;
		position = htonl(position);
		memcpy(&pdu->params[1], &position, sizeof(uint32_t));
		break;
						...
						uint8_t transaction)
{
	...
	case CAP_EVENTS_SUPPORTED:
		pdu->params_len = htons(7);
		pdu->params[1] = 5;
		pdu->params[2] = AVRCP_EVENT_STATUS_CHANGED;
		pdu->params[3] = AVRCP_EVENT_TRACK_CHANGED;
		pdu->params[4] = AVRCP_EVENT_TRACK_REACHED_END;
		pdu->params[5] = AVRCP_EVENT_TRACK_REACHED_START;
		pdu->params[6] = AVRCP_EVENT_PLAYBACK_POS_CHANGED;
						...
						uint8_t transaction)
{
	...
	case AVRCP_EVENT_TRACK_CHANGED:
		...
		if (uid != 0)
		    uid = ((uint64_t)htonl((uint32_t)uid)) << 32;
		else
		    uid = UINT64_MAX;
	...
	case AVRCP_EVENT_PLAYBACK_POS_CHANGED:
		len = 5;
		memcpy(&interval, &pdu->params[1], sizeof(uint32_t));
		interval = ntohl(interval);
		current = player->cb->get_position(player->user_data);
		player->last_sent_pos = current;
               current = htonl(current);
		memcpy(&pdu->params[1], &current, sizeof(uint32_t));
		g_timeout_add_seconds(interval, (GSourceFunc) notify_position, player);
		break;
	default:
